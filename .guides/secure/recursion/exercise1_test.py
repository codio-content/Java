...
def main():
    ...
    with open('.guides/secure/recursion/' + file, 'r') as student_code:
      ...
      if not(check_code_for_recursive_call(code_without_comments, 'recursiveSum', 'public static int recursiveSum')):
        print('Code did not recursively call the "recursiveSum" method')
        sys.exit(1)
      else:
        print('Code recursively called the "recursiveSum" method')
        sys.exit(0)
